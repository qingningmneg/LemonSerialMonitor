...
安装时选择了其他目录，就把以下示例中的路径换成实际安装位置。Server Core 可先运行 `%ProgramData%\LemonSerialMonitor\Installer\scripts\Get-CommMonitorStatus.ps1`，从输出的 `CoreRoot` 取得核心目录；AI 客户端位于该目录的 `ai\Lemon.SerialMonitor.AI.exe`。
...
没有接入串口设备时，`lemon_get_status` 仍应返回 `serviceState=available`、`driverState=unavailable`、`captureState=stopped`，警告可包含 `DriverUnavailable`，`lemon_list_ports` 返回空数组。这是可判定的无设备状态，不应被 AI 当成后台服务崩溃，也不需要反复重装；接入设备后重新调用状态和端口工具。
